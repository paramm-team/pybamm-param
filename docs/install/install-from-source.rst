...
This page describes the build and installation of PyBaMM-param from the source code, available on GitHub. Note that this is **not the recommended approach for most users** and should be reserved to people wanting to participate in the development of PyBaMM-param, or people who really need to use bleeding-edge feature(s) not yet available in the latest released version. If you do not fall in the two previous categories, you would be better off installing PyBaMM-param using pip or conda.
...
If you are running windows, you can simply skip this section and jump to :ref:`pybamm-param-install`.
...
.. code:: bash
	  ...
	  # in the PyBaMM-param/ directory
	  tox -e pybamm-param-requires
...
4. Clone the pybind11 repository in the ``PyBaMM-param/`` directory (make sure the directory is named ``pybind11``).
	 

PyBaMM-param ships with a Python script that automates points 2. and 3. You can run it with

.. code:: bash

	  python scripts/install_KLU_Sundials.py

.. _pybamm-param-install:

Installing PyBaMM-param
-----------------

You should now have everything ready to build and install PyBaMM-param successfully.
...
This creates a virtual environment ``.tox/dev`` (or ``windows-dev``) inside the ``PyBaMM-param/`` directory.
It comes ready with PyBaMM-param and some useful development tools like `flake8 <https://flake8.pycqa.org/en/latest/>`_ and `black <https://black.readthedocs.io/en/stable/>`_.
...
From the ``PyBaMM-param/`` directory, you can install PyBaMM-param using ``python setup.py install`` or 
...
If you intend to contribute to the development of PyBaMM-param, it is convenient to install in "editable mode", along with useful tools for development and documentation:
...
You can run unit tests for PyBaMM-param using
...
How to build the PyBaMM-param documentation
-------------------------------------
